...
class FFAppState extends ChangeNotifier {
  ...
  Future initializePersistedState() async {
    ...
    await _safeInitAsync(() async {
      _questionOne =
          await secureStorage.getInt('ff_questionOne') ?? _questionOne;
    });
    await _safeInitAsync(() async {
      _questionTwo =
          await secureStorage.getInt('ff_questionTwo') ?? _questionTwo;
    });
    await _safeInitAsync(() async {
      _questionThree =
    ...
    await _safeInitAsync(() async {
      _entryNumber =
          await secureStorage.getInt('ff_entryNumber') ?? _entryNumber;
    });
    await _safeInitAsync(() async {
      _positiveResult =
    ...
    await _safeInitAsync(() async {
      _selectedTime =
          await secureStorage.getString('ff_selectedTime') ?? _selectedTime;
    });
    await _safeInitAsync(() async {
      _dateToday = await secureStorage.read(key: 'ff_dateToday') != null
          ? DateTime.fromMillisecondsSinceEpoch(
              (await secureStorage.getInt('ff_dateToday'))!)
          : _dateToday;
    });
    await _safeInitAsync(() async {
      _scheduleLimit =
          await secureStorage.getString('ff_scheduleLimit') ?? _scheduleLimit;
    });
  }
  ...
  set selectedTime(String _value) {
    ...
    secureStorage.setString('ff_selectedTime', _value);
  }

  void deleteSelectedTime() {
    secureStorage.delete(key: 'ff_selectedTime');
  }

  DateTime? _dateToday;
  DateTime? get dateToday => _dateToday;
  set dateToday(DateTime? _value) {
    _dateToday = _value;
    _value != null
        ? secureStorage.setInt('ff_dateToday', _value.millisecondsSinceEpoch)
        : secureStorage.remove('ff_dateToday');
  }

  void deleteDateToday() {
    secureStorage.delete(key: 'ff_dateToday');
  }

  String _scheduleLimit = '';
  String get scheduleLimit => _scheduleLimit;
  set scheduleLimit(String _value) {
    _scheduleLimit = _value;
    secureStorage.setString('ff_scheduleLimit', _value);
  }

  void deleteScheduleLimit() {
    secureStorage.delete(key: 'ff_scheduleLimit');
  }
}
