...
String? sMSText(
  ...
  // print Hello, this is to request an appointment for {userName} on {selectedDate} at {selectedTime}
  if (userName == null || selectedDate == null || selectedTime == null) {
    return null;
  }
  return 'Hello, this is to request an appointment for ${userName} on ${selectedDate} at ${selectedTime}';
}

bool? appointmentSchedSlot(TherapyAppointmentRecord? appointmentRecord) {
  // if user has record for today, user cant make anymore entry and can only make one again tomorrow
  if (appointmentRecord == null) {
    return true;
  }
  if (appointmentRecord.hasSelectedDate()) {
    final DateTime now = DateTime.now();
    final DateTime today = DateTime(now.year, now.month, now.day);
    if (appointmentRecord.selectedDate == today) {
      return false;
    }
  }
  return true;
